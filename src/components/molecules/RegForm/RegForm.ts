...
export default class RegistrationForm {
  ...
  async renderStep3(): Promise<void> {
    ...
    const savedDay = Number(this.savedValues.bthDay) || 1;
    const savedMonthName = this.savedValues.bthMonth || MONTH_NAMES[0];
    const savedYear = Number(this.savedValues.bthYear) || maxYear;
    ...
    const savedGender = this.savedValues.gender || null;

    const checkboxes = genderContainer.querySelectorAll<HTMLInputElement>('input[name="gender"]');
    checkboxes.forEach((checkbox) => {
      if (savedGender && checkbox.value === savedGender) {
