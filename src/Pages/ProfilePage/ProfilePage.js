...
export async function getFriendsStatus(userId) {
    const response = await cachedFetch(
        `${process.env.API_BASE_URL}/api/friends/${userId}/status`,
...
export class ProfilePage extends BasePage {
    ...
    addListners() {
        const handleAddFriend = async (button) => {
            try {
                const response = await cachedFetch(
                    `${process.env.API_BASE_URL}/api/friends/${this.userId}`,
        ...
        if (addFriendBtn) addFriendBtn.addEventListener('click', () => handleAddFriend(addFriendBtn), { once: true });
        ...
        if (messageBtn) {
            messageBtn.addEventListener('click', async () => {
                try {
                    const response = await cachedFetch(
                        `${process.env.API_BASE_URL}/api/chats/user/${this.userId}`,
                        { credentials: 'include' },
                        "profile-cache-v1"
                    );
                    const data = await response.json();
        ...
        if (unblockButton) {
            unblockButton.addEventListener('click', () => {
                const blockConfirm = new ModalConfirm(
                    ...
                    async () => {
                        try {
                            notifier.show('Пользователь разблокирован', `Вы разблокировали пользователя ${this.profileData.firstName} ${this.profileData.lastName}`, "success");
